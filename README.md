...
For example, the `sensu_check` with name `check-cpu in qa` would be named `check-cpu` and put into the `qa` namespace.
...
The example above would add the `check-cpu` check to both the `default` and `qa` namespaces.
